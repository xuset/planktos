...
Snapshot.prototype.fetch = function (req, opts) {
  ...
  if (opts.inject != null ? opts.inject : req.inject) {
    bodyPromise = Promise.resolve(createInjection(req.url, opts.root, this.hash))
    responseMeta.headers['Content-Type'] = 'text/html'
  } else if (file != null) {
...
function createInjection (url, root, snapshotHash) {
  ...
  let html = (isHTML ? injection.docWrite : injection.rendermedia)
